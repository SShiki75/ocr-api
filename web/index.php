...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_FILES['receipt_image']) && $_FILES['receipt_image']['error'] === UPLOAD_ERR_OK) {
        ...
        error_log("URL: " . API_URL . '/scan');
        error_log("HTTP Code: " . $http_code);
        error_log("cURL Error: " . $curl_error);
        error_log("cURL Errno: " . $curl_errno);
        error_log("Connect Time: " . $curl_info['connect_time']);
        error_log("Total Time: " . $curl_info['total_time']);
        
        if (curl_errno($ch)) {
            $error = 'API接続エラー: ' . curl_error($ch) . " (Error #" . curl_errno($ch) . ")";
            error_log("cURL failed: " . $error);
        } elseif ($http_code !== 200) {
            $error = "APIエラー (HTTP $http_code): " . $response;
            error_log("API returned non-200: " . $error);
        } else {
...
<body>
    ...
    <script>
        ...
        document.getElementById('uploadForm').addEventListener('submit', function() {
            ...
            btn.textContent = '⏳ 処理中...（初回は1分ほどかかる場合があります）';
            btn.disabled = true;
            ...
            loadingDiv.innerHTML = `
                <div style="text-align: center; padding: 20px;">
                    ...
                    <p style="margin-top: 15px; font-weight: bold;">🔄 OCR処理中...</p>
                    <p style="font-size: 0.9em; color: #666;">
                        ※ 初回起動時は最大1分かかる場合があります<br>
                        そのままお待ちください
                    </p>
                </div>
            `;
            this.insertAdjacentElement('afterend', loadingDiv);
        });
    </script>
</body>
</html>
